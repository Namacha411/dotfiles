...
local claude_term_opts = { env = { CLAUDE_CODE_DISABLE_ALTERNATE_SCREEN = "1" } }
map('n', '<leader>wc', function()
  vim.cmd("vnew")
  vim.fn.termopen({ vim.fn.exepath("claude") }, claude_term_opts)
  vim.cmd("startinsert")
end, { desc = "縦分割でclaude codeを起動" })
map('n', '<leader>wC', function()
  vim.cmd("vnew")
  vim.fn.termopen({ vim.fn.exepath("claude"), "-c" }, claude_term_opts)
  vim.cmd("startinsert")
